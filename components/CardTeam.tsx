...
const CardTeam = ({ name, position, qualifications }: CardTeamProps) => {
    return (
        <div className="font-quicksand w-full">
            ...
            <div className='relative py-8 pl-6 pr-6 bg-blue-light text-black rounded-r-lg lg:h-full'>
                <h4 className='mb-4 text-button'>{name}</h4>
                ...
                <ul className='mt-16 flex flex-col gap-3'>
                    {qualifications.map((qual, index) => (
                        <li key={index} className='flex gap-3 items-start'>
                            <span>
                                <CheckIcon className='w-4 h-4 mt-1.5 text-secondary-blue' />
                            </span>
                            {qual}
                        </li>
                    ))}
